                    ...
                    {!productsList ? <p className="text-xs text-muted-foreground p-2">Loading products...</p> : productsList.map((p: any) => (
                      <div 
                        ...
                        className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer border transition-all ${
                          formData.products.includes(p._id) ? 'border-primary bg-primary/5 shadow-sm' : 'border-transparent hover:bg-muted'
                        }`}
                      >
                        <div className="w-10 h-10 bg-muted rounded overflow-hidden flex-shrink-0 relative">
                           {p.images?.[0] ? <Image src={`http://localhost:5000${p.images[0]}`} alt={p.name} fill className="object-cover" /> : <ImageIcon className="w-4 h-4 m-auto text-muted-foreground mt-3" />}
                        </div>
                  ...
                  <tr key={d._id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-6 py-4 flex items-center gap-4">
                      <div className="w-20 h-12 bg-background rounded overflow-hidden relative border border-border flex-shrink-0">
                        {d.image ? <Image src={`http://localhost:5000${d.image}`} alt={d.title} fill className="object-cover" /> : <ImageIcon className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />}
                      </div>
